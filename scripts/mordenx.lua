...
local user_opts = {
    ...
    ytdlpQuality =
    ''                                                         -- optional parameteres for yt-dlp downloading, eg: '-f bestvideo[vcodec^=avc][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best'
}
...
local osc_styles = {
    TransBg = "{\\blur100\\bord" ..
    user_opts.OSCfadealpha .. "\\1c&H000000&\\3c&H" .. osc_color_convert(user_opts.osc_color) .. "&}",
    ...
    user_opts.timefontsize .. '\\fn' .. user_opts.font .. '}',
    ...
    user_opts.descriptionfontsize .. '\\q2\\fn' .. user_opts.font .. '}',
                ...
                if not (element.slider.tooltipF == nil) then
                    if mouse_hit(element) then
                        ...
                        if element.thumbnailable and not thumbfast.disabled then
                            ...
                            if osd_w then
                                local hover_sec = 0
                                if (mp.get_property_number("duration")) then hover_sec = mp.get_property_number(
                                    "duration") * sliderpos / 100 end
...
function checktitle()
    ...
    if (mp.get_property("filtered-metadata/by-key/Album_Artist") and mp.get_property("filtered-metadata/by-key/Artist")) then
        if (mp.get_property("filtered-metadata/by-key/Album_Artist") ~= mp.get_property("filtered-metadata/by-key/Artist")) then
            ...
            ', ' .. mp.get_property("filtered-metadata/by-key/Artist")
        end
    end
...
function checkcomments()
    ...
    local ret = mp.command_native_async({
        ...
        local filename = mp.command_native({ "expand-path", user_opts.commentsdownloadpath .. '/' }) ..
        mp.get_property("filename"):gsub("watch%?v=", ""):match("^[^%?&]+") .. ".info.json"
        print(filename)
...
function exec_description(args, result)
    local ret = mp.command_native_async({
        ...
        state.localDescriptionClick = mp.get_property("media-title", "") ..
        string.gsub(string.gsub(val.stdout, '\r', '\\N') .. state.dislikes, '\n', '\\N')
        if (state.dislikes == "") then
            state.localDescriptionClick = mp.get_property("media-title", "") ..
            string.gsub(string.gsub(val.stdout, '\r', '\\N'), '\n', '\\N')
        ...
        if state.youtubeuploader then
            state.localDescriptionClick = state.localDescriptionClick:gsub("Uploader: <$\\N!uploader!\\N$>",
                "Uploader: " .. state.youtubeuploader)
        else
        ...
        if (state.localDescriptionClick:match('Views: (%d+)')) then
            state.localDescriptionClick = state.localDescriptionClick:gsub(
            state.localDescriptionClick:match('Views: (%d+)'), commas(state.localDescriptionClick:match('Views: (%d+)')))
        end
        if (state.localDescriptionClick:match('Likes: (%d+)')) then
            state.localDescriptionClick = state.localDescriptionClick:gsub(
            state.localDescriptionClick:match('Likes: (%d+)'), commas(state.localDescriptionClick:match('Likes: (%d+)')))
        end
        if (state.localDescriptionClick:match('Comments: (%d+)')) then
            state.localDescriptionClick = state.localDescriptionClick:gsub(
            state.localDescriptionClick:match('Comments: (%d+)'),
                commas(state.localDescriptionClick:match('Comments: (%d+)')))
        end
        ...
        if beforeLastPattern then
            ...
            if desc then
                ...
                afterLastPattern = afterLastPattern:gsub("Views:", emoticon.view):gsub("Comments:", emoticon.comment)
                :gsub("Likes:", emoticon.like):gsub("Dislikes:", emoticon.dislike)                                                                                                      -- replace with icons
...
function format_file_size(file_size)
    ...
    return string.format("%.1f %s", math.ceil(file_size), units[unit_index])
end
...
function show_description(text)
    ...
    local function returnMessageText()
        ...
        state.commentsPage .. '/' .. state.maxCommentPages .. ' (' .. lastCommentCount .. '/' .. totalCommentCount .. ')'
    end
...
layouts = function()
    ...
    -- Title
    geo = { x = 25, y = refY - 122 +
    (((state.localDescription ~= nil or state.isWebVideo) and user_opts.showdescription) and -20 or 0), an = 1, w =
    osc_geo.w - 50, h = 35 }
    lo = add_layout("title")
    lo.geometry = geo
    ...
    if showjump then
        ...
        lo.style = (user_opts.jumpiconnumber and jumpicons[user_opts.jumpamount] ~= nil) and osc_styles.Ctrl2 or
        osc_styles.Ctrl2Flip
    end
    ...
    if showscreenshot then
        ...
        lo.geometry = { x = osc_geo.w - 217 + (showloop and 0 or 45) + (showontop and 0 or 45) + (showinfo and 0 or 45), y =
        refY - 40, an = 5, w = 24, h = 24 }
        lo.style = osc_styles.Ctrl3
    ...
    if user_opts.downloadbutton then
        ...
        lo.geometry = { x = osc_geo.w - 262 + (showloop and 0 or 45) + (showontop and 0 or 45) + (showinfo and 0 or 45) +
        (showscreenshot and 0 or 45), y = refY - 40, an = 5, w = 24, h = 24 }
        lo.style = osc_styles.Ctrl3
...
function osc_init()
    ...
    ne.visible = (osc_param.playresx >= 1100 - outeroffset - (user_opts.showloop and 0 or 100) - (user_opts.showontop and 0 or 100) - (user_opts.showinfo and 0 or 100) - (user_opts.showscreenshot and 0 or 100)) and
    state.isWebVideo
    ne.tooltip_style = osc_styles.Tooltip
    ne.tooltipF = function()
...
if user_opts.keybindings then
    ...
    if (user_opts.showdescription) then
        mp.add_key_binding("d", "showdescription", checkDesc);
    end
